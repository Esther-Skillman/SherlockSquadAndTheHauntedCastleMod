...
local function placeEvidence()
    ...
    if owner.tags.hasTag('evidence') and owner.tags.hasTag('doll') then
        if Neighbour.tags.hasTag('ball') and Neighbour.tags.hasTag('picture') then
            ...
            print("ghost",carried.owner.hasFunc('cure') )
            if carried.owner.hasFunc('cure') then
                log:log('Calling cure on carried object')
                success = carried.owner.callFunc('cure', owner.gridPosition)
