...
local function displaySpeechFromChapter(chapter)
    ...
    local connectingLineYOffset = 1.0
    if carryable ~= nil and (not carryable.isAvailableToBeCarried) then
        connectingLineYOffset = 2.9
    end

    game.bus.send({
        metadata = { 'floatingUI.createOrUpdate' },
        data = {
            targetObject = owner.id,
            positionOffset = { y = connectingLineYOffset },
            displayData = 'speechBubble',
            speakerNameKey = 'characterName_' .. characterName,
...
local function hideSpeechBubble()
    if not showingSpeechBubble then
        return
    end
    game.bus.send({
...
local function onMapObjectStateChanged(message)
	...
	destroyed = true
    -- Ensure speech bubble goes away when owner is destroyed
    if showingSpeechBubble then
        hideSpeechBubble()
    end
end
