...
local function create(name)
    ...
    local newInstance = loader.instantiate("release_original", owner.gridPosition)
    prefabToNameMap[newInstance.id] = "release_original"
    log:debug('Created ', newInstance, ' (mapped "', newInstance.id, '" to "', name, '")')
    newInstance = loader.instantiate(name, owner.gridPosition)
    prefabToName[newInstance.id] = name
    log:debug('Created ', newInstance, ' (mapped "', newInstance.id, '" to "', name, '")')
...
local function onGamePhaseChanged(message)
    if message.data.gamePhase == 'planning' then
        ...
        penalise = true
        if health > 0 and startGame == true then
            showHealthIndicator()
            loseHealth(healthLostOnNewRound)
        end
    end
